...
public:
	...
	{
		return n1->m_data.pathCost + n1->m_data.CostReachGoalH > n2->m_data.pathCost + n2->m_data.CostReachGoalH;
	}
};
		...
		for (; iter != endIter; iter++)
		{
			...
			if (goalReached == false)
			{
				if (arc.node() == goal)
				{
					...
					std::cout << "Found: " << goal->m_data.m_name << std::endl;
				}
...
inline void Graph<NodeType, ArcType>::ucs(Node* start, Node* dest, std::function<void(Node*)> f_visit, std::vector<Node*>& path)
{
	...
	startingNode->m_data.m_pathCost = 0;
	pq.push(startingNode);
	startingNode->setMarked(true);

	//std::numeric_limits<int>::max;

	while (pq.size() != 0 && pq.top() != endingNode)
	{
		// add all of the child nodes that have not been 
	// marked into the queue
		...
		for (; iter != endIter; iter++)
		{
			...
			{
				int distC = arc.weight() + pq.top()->m_data.m_pathCost;
	...
	while (pq.size() != 0 && pq.top() != goalNode /**&& timer not expired*/&& m_timer.getElapsedTime().asMilliseconds() <= 50)
	{
		...
		for (; iter != endIter; iter++)
		{
			...
			if (arc.node() != pq.top()->previous() && pq.top()->m_data.passable == true)
			{
				...
				if (distC < arc.node()->m_data.pathCost)
				{
					arc.node()->m_data.pathCost = distC;
					arc.node()->setPrevious(pq.top());
				}
				if (arc.node()->marked() == false)
				{
